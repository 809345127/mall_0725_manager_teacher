...
<body>
	...
	<c:forEach items="${list_attr}" var="attr" varStatus="status">
		<input type="checkbox" name="list_av[${status.index}].shxm_id" value="${attr.id}" onchange="show_sku_check(${attr.id},this.checked)"/>${attr.shxm_mch}
	</c:forEach>
	<hr>
	<c:forEach items="${list_attr}" var="attr" varStatus="status">
		<div id="sku_attr_id_${attr.id}" style="display:none;">
			<c:forEach items="${attr.list_value}" var="val">
				<input type="radio" name="list_av[${status.index}].shxzh_id" value="${val.id}">${val.shxzh}${val.shxzh_mch}  
			</c:forEach>
		</div>
		<br>
	</c:forEach>
